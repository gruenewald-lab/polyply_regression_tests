...
polyply gen_itp -lib martini3 -f ${INPUT_DIR}/lipids.itp -name PEL -seq POPE:1 PEO:45 -o PEL.itp
...
mkdir dextran_itps
python3 ${INPUT_DIR}/random_lin_comb.py "dextran_graphs/" 451
...
#include "${FF_DIR}/martini3/martini_v3.0.0_ions_v1.itp"
#include "${INPUT_DIR}/lipids.itp"
...
polyply gen_coords -p system.top -c ${INPUT_DIR}/vesicle.gro -res PEO W GLC -o start.gro -name test -box 65 65 65 -b build_file.bld -split POPE:HEAD-NH3:TAIL-PO4,GL1,GL2,C1A,D2A,C3A,C4A,C1B,C2B,C3B,C4B

${INPUT_DIR}/SOL -in start.gro -o solvated.gro -tem ${INPUT_DIR}/W.gro
cat info.txt >> system.top

gmx grompp -f ${INPUT_DIR}/min_TI.mdp -c solvated.gro -p system.top -maxwarn 1
gmx mdrun -v -nsteps 50

gmx grompp -f ${INPUT_DIR}/min.mdp -c confout.gro -p system.top -maxwarn 1
gmx mdrun -v
